...
test.describe('API Tests', async () => {
    ...
    test('Assert Pet can be added successfully', async () => {
        const response = await apiHelper.addPet(
            apiContext,
            `pet`,
            {...petData, id: petId, name: `Labrador ${petName}`}
        );
    ...
    test('Assert find Pet by Id call can be made', async () => {
        const response =  await apiHelper.findPetById(
            apiContext,
            `pet/${petId}`
        );
       expect(response.ok()).toBeTruthy();
    ...
    test('Assert PetName can be updated successfully', async () => {
        ...
        const response = await apiHelper.addPet(
            apiContext,
            `pet`,
            {...petData, id: petId, name: `Labrador ${updatedName}`}
        );
       expect(response.ok()).toBeTruthy();
       const petDetails = await response.json()
    ...
    test('Assert pet can be deleted successfully', async () => {
        const response =  await apiHelper.deletePet(
            apiContext,
            `pet/${petId}`
        );
        expect(response.ok()).toBeTruthy();

        const missingPetResponse = await apiHelper.findPetById(
            apiContext,
            `pet/${petId}`
        );
